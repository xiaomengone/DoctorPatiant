...
import type { TabItemList } from "@/types/home";
import { useFollowInterestLogic } from "@/composables";

defineProps<{
  items: TabItemList[];
}>();
const { loading, followInterest } = useFollowInterestLogic();
</script>
  ...
  >
    <div class="head">
      ...
      <van-button
        ...
        size="small"
        round
        @click="followInterest('knowledge', item)"
        :loading="loading"
        >{{ item.likeFlag == 0 ? "+ 关注" : "已关注" }}</van-button
      >
    </div>
    <div class="body">
      ...
      <div class="imgs">
        <van-image
          :src="url"
          v-for="(url, index) in item.coverUrl"
          :key="index"
        />
      </div>
      <p class="logs">
